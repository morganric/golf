class HolesController < ApplicationController
  ...
  # GET /holes/1.json
  def show
    @club = @hole.course.club

    @coords = @hole.coords

    @coordsArray = Array.new

    @hole.coords.each do |coord|
      @coordsArray << {:lat => coord[:latitude], :long => coord[:longitude]}
    end
  end
  ...
  private
    ...
    def hole_params
      params.require(:hole).permit(:number, :course_id, :image, :stroke_index, :video_embed_code, :slug, :name)
    end
end
